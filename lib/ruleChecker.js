...
import { allRulesDisabled, formatters, loadMarkdownlintConfig } from "./utils/index.js";
...
// MD052 and MD053 use "parser: none" but depend on the micromark cache,
// which is only populated when at least one micromark-based rule runs.
// MD042 uses micromark and serves as the required companion.
const MICROMARK_CACHE_RULES = new Set(["md052", "md053"]);
const MICROMARK_COMPANION = "md042";

const check = (ruleId, context, src) => {
  const needsCompanion = MICROMARK_CACHE_RULES.has(ruleId);

  // Build rule options: config file options < ESLint rule options
  const customConfigFile = context.settings?.markdownlint?.configFile;
  const fileConfig = loadMarkdownlintConfig(
    context.filename ?? context.getFilename?.() ?? "",
    customConfigFile
  );
  const fileRuleOptions = fileConfig?.[ruleId.toUpperCase()] ?? fileConfig?.[ruleId];
  const eslintOptions = context.options?.[0];

  let ruleOptions;
  if (eslintOptions != null) {
    // ESLint options take precedence; merge on top of file config if it's an object
    ruleOptions =
      typeof fileRuleOptions === "object" && fileRuleOptions !== null
        ? Object.assign({}, fileRuleOptions, eslintOptions)
        : Object.assign({}, eslintOptions);
  } else if (fileRuleOptions != null) {
    // No ESLint options — use config file options
    ruleOptions =
      typeof fileRuleOptions === "object"
        ? Object.assign({}, fileRuleOptions)
        : fileRuleOptions;
  } else {
    ruleOptions = true;
  }
  ...
  const config = Object.assign({}, allRulesDisabled, {
    [ruleId]: ruleOptions,
    ...(needsCompanion ? { [MICROMARK_COMPANION]: true } : {}),
  });

  const { text: allErrors } = lint({
    strings: { text: src.getText() },
    config,
  });

  const errors = needsCompanion
    ? allErrors.filter((e) => e.ruleNames[0].toLowerCase() === ruleId)
    : allErrors;
...
const getRange = (src, err) => {
  ...
  if (err.fixInfo === null) {
    ...
    const lineNumber = Math.min(err.lineNumber - err.fixInfo.deleteCount, text.length);
    end = src.getIndexFromLoc({
      line: lineNumber,
      column: 0,
    });
